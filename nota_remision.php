...
$pdf->SetFillColor(93, 186, 213);
$pdf->Cell(90, 10, 'NOTA DE REMISION', 1, 1, 'C',1);
...
if ($sql && $sql->num_rows > 0) {
    ...
    $pdf->Cell(40, 10, '$' . number_format($total, 2), 1, 1, 'C', 1);


    $pdf->SetFillColor(0, 159, 210);
    $pdf->Ln(2);
    $resultado = $conexion->query("SELECT pago FROM detalleventas");
    $fila = $resultado->fetch_assoc();
    $pago = $fila['pago'];
    $pdf->Cell(50, 10, 'Pago', 1, 0, 'C', 1);
    $pdf->Cell(40, 10, '$' . number_format($pago, 2), 1, 1, 'C', 1);
    
    $pdf->SetFillColor(35, 190, 94);
    $pdf->Ln(2);
    $cambio = $pago - $total;
    $pdf->Cell(50, 10, 'Cambio', 1, 0, 'C', 1);
    $pdf->Cell(40, 10, '$' . number_format($cambio, 2), 1, 1, 'C', 1);
