...
$pdf->SetFillColor(226, 117, 38);
$pdf->SetFont('Arial', '', 10, 'C');
$pdf->Cell(90, 10, 'NOTA DE COMPRAS DE PRODUCTOS', 1, 1, 'C',1);
...
$sql = $conexion->query("SELECT descripcion_producto, cantidad_comprada, precio_unitario, subtotal, fecha, nombre_proveedor
                         ...
                         ORDER BY id_detallecompra DESC");
...
if ($sql && $sql->num_rows > 0) {
    ...
    $pdf->Cell(30, 10, 'Fecha y Hora', 1, 0, 'C', 1);
    $pdf->Cell(40, 10, 'Proveedor', 1, 1, 'C', 1);


    // Agrega los datos de la tabla al PDF
    while ($datos = $sql->fetch_object()) {
        $pdf->Cell(50, 10, remove_junk($datos->descripcion_producto), 1);
        $pdf->Cell(20, 10, remove_junk($datos->precio_unitario), 1);
        $pdf->Cell(20, 10, remove_junk($datos->cantidad_comprada), 1);
        $pdf->Cell(20, 10, remove_junk($datos->subtotal), 1);
        $pdf->Cell(30, 10, date('Y-m-d H:i:s', strtotime($datos->fecha)), 1);
        $pdf->Cell(40, 10, remove_junk($datos->nombre_proveedor), 1);
        $pdf->Ln();
    }
    ...
    $pdf->Cell(50, 10, 'Utilidad', 1);
    $pdf->Cell(40, 10, '$' . number_format($utilidad_total, 2), 1);
     */   
    ...
    echo '<div style="background-color: #F8BB00; font-size:4rem; text-align:center; margin: 0; padding: 0; box-sizing: border-box;
    font-family: sans-serif;">¡Porfavor, Registre Compras!</div>';
}
