...
<head>
    ...
    <title>🛠️ Ferreteria | Marcas</title>
    <!--Bootstrap CSS-->
...
<body>
    ...
    <div class="container-fluid row d-flex justify-content-center">
        <form class="col-sm-1 col-md-3 pt-3" method="POST" onsubmit="convertirAMayusculas()">
            <h3 class="text-center color_h3">Registro de Marcas</h3>
            <?php
            include "model/conexion.php";
            include "controller/eliminar_marcas.php"
            ?>

            <?php 
            include "controller/registro_marcas.php";
            ?>

            <div class="mb-1 mt-3">
              <label for="exampleInputEmail1" class="form-label">Marcas:</label>
              <input type="text" class="form-control" name="marcas" onkeyup="mayus(this);">
            </div>
        ...
        <div class="col-sm-1 col-md-3 pt-4" style="max-height: 500px; overflow-y: auto;">
            <table class="table">
                ...
                <tbody>
                    <?php 
                    include "model/conexion.php";                    
                    $sql = $conexion->query("SELECT * FROM marcas c WHERE c.estado = 1 ORDER BY c.id_marca DESC;");
                    while ($datos = $sql->fetch_object()) { ?>
                        <tr>
                            <th><?php echo $datos->id_marca; ?></th>
                            <td><?php echo $datos->nombre_marca; ?></td>
                            <td>
                                <a href="modificar_marcas.php?id=<?= $datos->id_marca; ?>"><ion-icon style="color: #843C0C; font-size:2rem; background-color: #E27526; padding:1px; border-radius: 5px;" name="create-outline"></ion-icon></a>
                                <a href="marcas.php?id=<?= $datos->id_marca; ?>"><ion-icon style="color: #843C0C; font-size:2rem; background-color: #E27526; padding:1px; border-radius: 5px;" name="trash-outline"></ion-icon></a>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
